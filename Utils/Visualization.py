...
def flow2Grid(flow, step, plt, c='yellow'):
    img_h = flow.shape[1]
    img_w = flow.shape[2]
    ...
    for i in range(0, img_h, step):
        line_point = flow[:, i, :]
        plt.plot(line_point[0, :], line_point[1, :], c=c, linewidth=1)
...
def saveImage(copy_plt, save_path):
    copy_plt.axis('off')
    ...
    copy_plt.savefig(save_path, bbox_inches='tight', pad_inches=0.0)